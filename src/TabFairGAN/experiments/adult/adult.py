...
def train_plot(df, epochs, batchsize, fair_epochs, lamda):
    generator, critic, ohe, scaler, data_train, data_test, input_dim = train(
        df, epochs, batchsize, fair_epochs, lamda
    )
    return generator, critic, ohe, scaler, data_train, data_test, input_dim
...
def multiple_runs(num_trainings, df, epochs, batchsize, fair_epochs, lamda, outFile):
    ...
    for i in range(num_trainings):
        if i == 0:
            first_line = (
                "num_trainings: %d, num_epochs: %d, batchsize: %d, fair_epochs:%d, lamda:%f"
        ...
        df_generated = (
            generator(torch.randn(size=(32561, input_dim), device=device))
            .cpu()
            .detach()
            .numpy()
        )
...
multiple_runs(
    ...
    fair_epochs=args.num_fair_epochs,
