...
def test_ctgan_no_categoricals():
    ...
    assert list(ctgan.loss_values.columns) == [
        "Epoch",
        "Generator Loss",
        "Discriminator Loss",
    ]
...
def test_log_frequency():
    ...
    pd.testing.assert_series_equal(
        ctgan.loss_values["Epoch"], pd.Series(range(100), name="Epoch")
    )
...
def test_wrong_sampling_conditions():
    ...
    with pytest.raises(
        ValueError, match="The column_name `cardinal` doesn't exist in the data."
    ):
